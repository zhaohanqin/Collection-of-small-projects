...
class handDetector():
    ...
    def findAllHands(self, img, draw=True):
        """
        检测所有手部并返回每只手的信息
        返回格式: [{'lmList': [...], 'bbox': [...], 'handType': 'Left'/'Right'}, ...]
        """
        ...
        if self.results.multi_hand_landmarks:
            for handLms, handInfo in zip(self.results.multi_hand_landmarks, self.results.multi_handedness):
                # 获取手部类型（左手或右手）
                handType = handInfo.classification[0].label  # "Left" 或 "Right"
                ...
                if draw:
                    ...
                    # 在手部上方显示左右手标识
                    cv2.putText(img, handType, (bbox[0] - 20, bbox[1] - 30), 
    ...
    def findDistance(self, p1, p2, img, lmList=None, draw=True):
        """
        计算两个关键点之间的距离
        如果提供了lmList，使用该列表，否则使用self.lmList
        """
        if lmList is None:
            lmList = self.lmList
        ...
        x1, y1 = lmList[p1][1], lmList[p1][2]# 获取第一个点的x和y坐标
        x2, y2 = lmList[p2][1], lmList[p2][2]# 获取第二个点的x和y坐标
    ...
    def getHandReferenceLength(self, lmList):
        """
        获取手掌参考长度，用于距离归一化
        使用手掌底部到中指根部的距离作为参考
        """
        ...
        # 手掌底部点 (0) 到小指根部点 (17) 的距离
        ref_x1, ref_y1 = lmList[0][1], lmList[0][2]
        ref_x2, ref_y2 = lmList[17][1], lmList[17][2]
        ref_length = math.hypot(ref_x2 - ref_x1, ref_y2 - ref_y1)
...
def main():
    ...
    while True:
        ...
        allHands, img = detector.findAllHands(img)# 找到所有手
        
        if len(allHands) != 0:
            # 打印检测到的手部信息
            for hand in allHands:
                print(f"检测到{hand['handType']}手")
